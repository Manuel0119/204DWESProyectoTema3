        ...
        <div class="codigophp">
            ...
            $aRespuestas=[
                ...
                if (isset($_REQUEST['enviar'])){
                    $aErrores["Nombre"]=validacionFormularios::comprobarAlfabetico($_REQUEST['nombre'],300, 1, 1);//Validación del nombre
                    $aErrores["Altura"]=validacionFormularios::comprobarEntero($_REQUEST['altura'], 280, 0, 1);//Validación de la altura
                ...
                if ($entradaOk){
                    ...
                    $aRespuestas["Altura"]=$_REQUEST['altura']." cm";
                    foreach ($aRespuestas as $claveRespuesta=>$valorRespuesta){
                        print("$claveRespuesta : $valorRespuesta <br>");
                    }
                }else{
            ?>
                ...
                <form name="ejercicio24" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
                        <label for="nombre">Nombre:
                            <input type="text" name="nombre" class="entradadatos" value="<?php echo $_REQUEST['nombre'] ?? ''; ?>"/><!-- Nombre del usuario. -->
                            <!--Mostrar los errores.-->
                            <?php echo '<span style="color: red;">'.$aErrores["Nombre"].'</span>'."<br><br>";?>
                        </label>
                        <label for="altura">Altura(cm):
                            <input type="text" name="altura" class="entradadatos" value="<?php echo $_REQUEST['altura'] ?? ''; ?>"/><!-- Altura del usuario. -->
                            <!--Mostrar los errores.-->
